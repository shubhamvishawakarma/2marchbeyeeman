...
const indexPage=async(req,res)=>{
   ...
   //create admin indexpage api
    ...
    const e={productcount};
        ...
        const dinesh= await User.findOne({is_user:1});
        const userId=dinesh._id;
        const dinu=await Notification.find({userId:userId}); 
       const counts=await Notification.find({userId:userId}).count();
       const f={counts};
        const data=[a,b,c,d,e,f];
    res.render('index',{data:data,datas:dinu});
};
...
const adminNotificationList=async(req,res)=>{
    try{
    const data= await User.findOne({is_user:1});
    const userId=data._id;
    const notificationData=await Notification.find({userId:userId});
    if(!notificationData){
        res.status(400).json({"result":"false","message":"data does not found"});
    }else{
        res.status(200).json({"result":"true",data:notificationData});
    }
